...
async function LLM(prompt, systemMessage, temperature, max_tokens) {
  try {
...
Ticket Info:
${JSON.stringify(extractTicketDetails())}

Ticket Comments Info:
${JSON.stringify(extractAllCommentsDetails())}
...
please take the next message as your directions from the technician on how to write the message.`;
        ...
        try {
          const response = await LLM(textContent, systemMessage, 0.7, 15000);
          v.removeClass("disabled");
          v.text("Generate Response");
          $("#gpt-response-container").show();
          $("#gpt-accept").removeClass('disabled');
          $("#gpt-output").val(response.message.content);
        } catch (error) {
          v.removeClass("disabled");
          v.text("Generate Response");
