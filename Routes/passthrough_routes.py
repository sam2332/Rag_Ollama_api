...
def register_routes(app):
    ...
    async def passthrough_chat(data: ChatPassthroughRequest):
        ...
        logging.info(f"Recieved {response.status_code} response from chat request: {response.text}")
        ...
        if response.status_code == 200:
            ...
            raise HTTPException(status_code=response.status_code, detail="Error processing chat request with external model")
    ...
    async def rag_passthrough_chat(data: ChatPassthroughRagRequest):
        ...
        with get_db_connection(data.embeddings_model_db) as conn:
            with closing(conn.cursor()) as cursor:
                ...
                cursor.execute("SELECT source, content, embedding FROM embeddings where ")
    ...
    ```"""
                ...
                response = requests.post(app.config.get('ollama_host')+"/api/chat", json={
                    "stream": False,
