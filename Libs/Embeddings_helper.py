...
def insert_embedding(embeddings_db, content, source, check_existing=True):
    print(f"Inserting into {embeddings_db} embedding for {len(content)} bytes from {source}")
    ...
    if response.status_code == 200:
        ...
        with get_db_connection(embeddings_db) as conn:
            with closing(conn.cursor()) as cursor:
                ...
                if check_existing:
                    cursor.execute("SELECT * FROM embeddings WHERE source = ? AND content = ?", (source, content))
                    if cursor.fetchone():
                        return {"status": "existing", "content": content, "embedding": embedding}
